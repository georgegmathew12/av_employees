...
| Dates stored as `MM/DD/YY` VARCHAR | Two-digit year is ambiguous | Project sets Snowflake `TWO_DIGIT_CENTURY_START = 1925` via `pre_hook` on every model/test/snapshot session: YY 25-99 → 1900s, 00-24 → 2000s | Source supplies ISO dates → drop the macro + pre_hook |
